...
#PART I: CALCULATE INPUTS FOR TRANSPORT CONSTRAINTS
...
#PART II: Add Exchange Reactions
...
reaction.add_metabolites({ATP_COMM_e: -1*Total_Abnd})
...
glc4_e = Metabolite('glc4_e',formula='C24H48O24',name='glucan',compartment='e', charge=0)
...
reaction.add_metabolites({xyl4_e: SEO_Abnd,
                          xyl4_SEOe: -1})   #^ what does the SEO_Abnd mean? why is it not a number?
...
glc4_SEOc = Metabolite('glc4_SEOc',formula='C24H48O24',name='glucan',compartment='SEOc', charge=0)

glc4_SEOe = Metabolite('glc4_SEOe',formula='C24H48O24',name='glucan',compartment='SEOe', charge=0)
...
reaction.add_metabolites({glc4_SEOe: -1.0,
                          glc4_e: SEO_Abnd})    #^ again not really sure why it's not a number
...
#This reaction differs from BiGG database because a different ferredoxin is used and H+ is a product for mass and charge balance
#^ this same description is used for the reaction above, and I just wanted to make sure that the statement is true for both cases
...
glc4_SFOc = Metabolite('glc4_SFOc',formula='C24H48O24',name='glucan',compartment='SFOc', charge=0)

glc4_SFOe = Metabolite('glc4_SFOe',formula='C24H48O24',name='glucan',compartment='SFOe', charge=0)
...
glc4_HSFc = Metabolite('glc4_HSFc',formula='C24H48O24',name='glucan',compartment='HSFc', charge=0)

glc__D_HSFc = Metabolite('glc__D_HSFc', formula='C6H12O6',name='D-Glucose',compartment='HSFc', charge=0)

glc4_HSFe = Metabolite('glc4_HSFe',formula='C24H48O24',name='glucan',compartment='HSFe', charge=0)
...
print(reaction.name + ": " + str(reaction.check_mass_balance()))   #^ check to see if this was a copy or if needed for code
...
"""###Ethanol Elongating Organism (EEO)###
...
print(reaction.name + ": " + str(reaction.check_mass_balance()))  #^ check to see if doubled model and print
...
#PART IV: SET ADDITIONAL CONSTRAINTS, OBJECTIVE FUNCTION, AND RUN MODEL WITH pFBA
...
"""#ATP Yield Equal to Guild Abundance
Constraint_abundance_SEO = model.problem.Constraint(model.reactions.SEO_ATP_Hydrolysis.flux_expression  - model.reactions.EX_ATP_COMM_e.flux_expression*SEO_Abnd, lb=-SEO_Abnd*0.1, ub=SEO_Abnd*0.1)
model.add_cons_vars(Constraint_abundance_SEO)

Constraint_abundance_SFO = model.problem.Constraint(model.reactions.SFO_ATP_Hydrolysis.flux_expression  - model.reactions.EX_ATP_COMM_e.flux_expression*SFO_Abnd, lb=-SFO_Abnd*0.1, ub=SFO_Abnd*0.1)
model.add_cons_vars(Constraint_abundance_SFO)

Constraint_abundance_HSF = model.problem.Constraint(model.reactions.HSF_ATP_Hydrolysis.flux_expression  - model.reactions.EX_ATP_COMM_e.flux_expression*HSF_Abnd, lb=-HSF_Abnd*0.1, ub=HSF_Abnd*0.1)
model.add_cons_vars(Constraint_abundance_HSF)

Constraint_abundance_LEO = model.problem.Constraint(model.reactions.LEO_ATP_Hydrolysis.flux_expression  - model.reactions.EX_ATP_COMM_e.flux_expression*LEO_Abnd, lb=-LEO_Abnd*0.1, ub=LEO_Abnd*0.1)
model.add_cons_vars(Constraint_abundance_LEO)"""

#Same ATP Yield  #^ I'm not sure what this comment is for/what constraints are being achieved
...
Fract = 1  #^ what is this calculating?

model.reactions.EX_octa_e.upper_bound = 0.0034 * (1+(1-Fract))
model.reactions.EX_octa_e.lower_bound = 0.0034 * Fract

model.reactions.EX_hxa_e.upper_bound = 0.0478 * (1+(1-Fract))
model.reactions.EX_hxa_e.lower_bound = 0.0478 * Fract

model.reactions.EX_but_e.upper_bound = 0.0827 * (1+(1-Fract))
model.reactions.EX_but_e.lower_bound = 0.0827 * Fract

model.reactions.EX_ac_e.lower_bound = 0.0740 * (1+(1-Fract))
model.reactions.EX_ac_e.upper_bound = 0.0740 * Fract

model.reactions.EX_etoh_e.lower_bound = 0.0032 * (1+(1-Fract))
model.reactions.EX_etoh_e.upper_bound = 0.0032 * Fract

model.reactions.EX_for_e.lower_bound = 0.0001 * (1+(1-Fract))
model.reactions.EX_for_e.upper_bound = 0.0001 * Fract

model.reactions.EX_h2_e.lower_bound = 0.437 * (1+(1-Fract))
model.reactions.EX_h2_e.upper_bound = 0.437 * Fract

model.reactions.EX_lac__D_e.upper_bound = 0 * (1+(1-Fract))
model.reactions.EX_lac__D_e.lower_bound = 0 * Fract
...
#PART VI: CALCULATE OVERALL THERMODYNAMICS
...
G_GLC4 = pfba_solution["EX_glc4_e"]*-694.61*4.184
...
#PART VII: PRINT RESULTS TO EXCEL
#writer = pandas.ExcelWriter('iFermentCommunity_Reactor_FVA.xlsx')
